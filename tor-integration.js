...
function getNextProxy() {
  ...
  const proxy = config.proxies[stats.currentProxyIndex % config.proxies.length];
  stats.currentProxyIndex++;
  return proxy;
}

// Check if we need to rotate IP
async function checkAndRotateIP() {
  if (
    stats.processed > 0 &&
    stats.processed % config.ipRotationInterval === 0
  ) {
    console.log("IP rotation needed after", stats.processed, "ads");
    ...
    console.log("Browser reinitialized with new IP");
    await bot.sendMessage(
      config.telegramChatId,
      `🔄 IP rotated after ${stats.processed} ads`
    );
  }
}
...
const config = {
  ...
  // Performance
  maxConcurrentPages: 3, // Reduced for Tor (slower)
  batchSize: 30, // Reduced batch size for Tor
  notificationInterval: 10,
  ipRotationInterval: 20, // Rotate IP every 20 ads
  ...
  // Proxy options
  useTor: true, // Set to true to use Tor
  torSocksPort: 9050,
  torControlPort: 9051,

  // Regular proxies (if not using Tor)
  proxies: [
...
async function extractListingDetails(page, url) {
  try {
    ...
    console.log("Page scrolled to load all content");
    ...
    const details = await page.evaluate(() => {
      ...
      const data = {
        ...
        location: getText(
          "section.container-fluid div.flex div.text-gray__dark_2 p.text-body_2.truncated-text"
        ),
        price: getText(
    ...
    try {
      ...
      const callButtonExists = await page.$(
        "section#stickyDiv button.\\!bg-blue"
      );
      ...
      if (!callButtonExists) {
        ...
        // Click call button
        await page.click("section#stickyDiv button.\\!bg-blue");
        ...
        try {
          ...
          // Extract phone numbers (multiple if exist)
          const phoneNumbers = await page.evaluate(() => {
            ...
            if (phoneEls.length > 0) {
              return Array.from(phoneEls).map((el) => el.textContent.trim());
            }
            // Fallback to single phone selector
            const singlePhone = document.querySelector(
              'div.flex-col.bg-white.w-full div.justify-items-center.w-100 button[aria-label="show-tooltip"] span.text-title_4'
            );
            return singlePhone ? [singlePhone.textContent.trim()] : [];
          });

          phoneNumber = phoneNumbers.length > 0 ? phoneNumbers : null;
      ...
      try {
        ...
        if (whatsappButtonExists) {
          ...
          try {
            ...
            // Get WhatsApp numbers (multiple if exist)
            const whatsappNumbers = await page.evaluate(() => {
              const whatsappEls = document.querySelectorAll(
                "dialog#listingPhones button.justify-between span.text-title_4"
              );
              if (whatsappEls.length > 0) {
                return Array.from(whatsappEls).map((el) =>
                  el.textContent.trim()
                );
              }
              // Fallback to single phone selector
              const singleWhatsapp = document.querySelector(
                'div.flex-col.bg-white.w-full div.justify-items-center.w-100 button[aria-label="show-tooltip"] span.text-title_4'
              );
              return singleWhatsapp ? [singleWhatsapp.textContent.trim()] : [];
            });

            whatsappNumber =
              whatsappNumbers.length > 0 ? whatsappNumbers : null;
            console.log("WhatsApp numbers:", whatsappNumber);
  ...
  try {
    ...
    for (const listing of listings) {
      ...
      try {
        ...
        await checkAndRotateIP();

        // Create new page if browser was rotated
